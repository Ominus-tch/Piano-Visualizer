...
MIDISceneLive::MIDISceneLive(int port, bool verbose) : MIDIScene() {
	...
	if (port >= 0) {
		shared().open_port(port, "PianoVisualizer input");
		_deviceName = _availablePorts[port];
	}
	else {
		shared().open_virtual_port("PianoVisualizer virtual input");
...
void MIDISceneLive::updatesActiveNotes(double time, double speed, const FilterOptions& filter) {
	...
	while (true) {
		...
		if (message.is_note_on_or_off()) {
			...
			if (_verbose) {
				std::cout << "Control: " << rawType << "(" << message.timestamp << "): " << val << "\n";
			}
			if (_audioEngine)
				_audioEngine->controlChange((int16_t)message.get_channel(), (int16_t)rawType, val);
		}
		else {
			if (_verbose) {
...
libremidi::midi_in& MIDISceneLive::shared() {
	if (_sharedMIDIIn == nullptr) {
		_sharedMIDIIn =
			new libremidi::midi_in(libremidi::API::UNSPECIFIED, "PianoVisualizer");
	}
